...
class IndexTTS:
    ...
    # Fast inference: For multi-sentence long text, can achieve at least 2~10x speedup (First modified by sunnyboxs 2025-04-16)
    def infer_fast(self, audio_prompt, text, output_path, verbose=False):
        ...
        # Only regenerate cond_mel if reference audio changes, to improve speed
        ...
        bucket_enable = True # Pre-bucketing switch, prioritize quality=True, prioritize speed=False.
        ...
        # clear cache
        tqdm_progress.close()  # Ensure progress bar is closed
        chunk_latents.clear()
        ...
        wav = wav.cpu()  # to cpu
        if output_path:
            # Directly save audio to specified path
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            torchaudio.save(output_path, wav.type(torch.int16), sampling_rate)
            print(">> wav file saved to:", output_path)
            return output_path
        else:
            # Return in format compatible with Gradio
