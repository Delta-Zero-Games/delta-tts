...
import warnings
from deltatts.utils.common import tokenize_by_CJK_char, de_tokenized_by_CJK_char
...
class TextNormalizer:
    ...
    def match_email(self, email):
        # Regex to match email format: alphanumeric@alphanumeric.alpha
        pattern = r"^[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[a-zA-Z]+$"
        return re.match(pattern, email) is not None

    """
    Match pinyin tone format: pinyin+number, tone 1-5, 5 means neutral tone
    Example: xuan4, jve2, ying1, zhong4, shang5
    """
    PINYIN_TONE_PATTERN = r"([bmnpqdfghjklzcsxwy]?h?[aeiouüv]{1,2}[ng]*|ng)([1-5])"
    """
    Match Chinese names, format: Chinese·Chinese, Chinese·Chinese-Chinese
    Example: 克里斯托弗·诺兰, 约瑟夫·高登-莱维特
    """
    ...
    def correct_pinyin(self, pinyin: str):
        """
        Translate pinyin for jqx with finals u/ü to v
        E.g. ju -> jv , que -> qve, xün -> xvn
        """
        if pinyin[0] not in "jqxJQX":
            return pinyin
        # Match pinyin for jqx with finals u/ü
    ...
    def save_names(self, original_text):
        ...
        # Replace with placeholders <n_a>, <n_b>, ...
    ...
    def restore_names(self, normalized_text, original_name_list):
        """
        Restore names from placeholders to original text
    ...
    def save_pinyin_tones(self, original_text):
        """
        Replace pinyin tones with placeholders <pinyin_a>, <pinyin_b>, ...
    ...
    def restore_pinyin_tones(self, normalized_text, original_pinyin_list):
        """
        Restore pinyin tone numbers (1-5) to original pinyin
...
class TextTokenizer:
    ...
    def split_sentences_by_token(
        ...
        """
        Split tokenized results by specific tokens
        """
        ...
        for i in range(len(tokenized_str)):
            ...
            if token in split_tokens:
                if len(current_sentence) == 1:
                    # If the current token is the only token and is a split token, ignore this sentence
                    pass
                elif len(current_sentence) == 2 and current_sentence[0] == '▁':
                    # If the current tokens only have two and only the split token, ignore this sentence
                    pass
                elif len(current_sentence) <= max_tokens_per_sentence:
                    if i < len(tokenized_str) - 1:
                        if tokenized_str[i + 1] in ["'", "▁'"]:
                            # If the next token is '，then do not split
                            current_sentence.append(tokenized_str[i + 1])
                            i += 1

                    sentences.append(current_sentence)
                else:
                    # If the length of current tokens exceeds the maximum limit
                    if "," in current_sentence or "▁," in current_sentence: 
                        # If the current tokens contain a comma, split by comma
                        ...
                        # If the current tokens contain a hyphen, split by hyphen
                        sub_sentences = TextTokenizer.split_sentences_by_token(
                            current_sentence, ["-"], max_tokens_per_sentence=max_tokens_per_sentence
                        )
                    else:
        ...
        if len(current_sentence) > 0:
            sentences.append(current_sentence)
        # If adjacent sentences together are less than the max limit, merge them
        merged_sentences = []
...
if __name__ == "__main__":
    ...
    # There should not be unk_token_id
